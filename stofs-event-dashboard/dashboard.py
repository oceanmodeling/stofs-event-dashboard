...
def plot_ts(event):
    storm = UI_storm.storm.value
    plot_type = UI.plot_type.value
    fc_type = UI.forecast_type.value
    modvars = UI_toggle.models_vars.value
    station = UI.station.value
    percentile = UI.quantile.value
    window = UI.window.value
    quantile = percentile / 100
    title = f"{storm}: {fc_type} at {station}"
    ylabel = get_plot_label(plot_type)
    try:
        # obs
        obs = load_data(DATA_DIR.joinpath(storm, plot_type, 'obs', station +'.parquet')).iloc[:,0]
        ...
        return pn.pane.Str(f'{title}: No {plot_type} observation data')
...
def plot_table_comparison(event):
    ...
    try:
        obs = load_data(DATA_DIR.joinpath(storm, plot_type, 'obs', station +'.parquet')).iloc[:,0]
        start, end = obs.index.min(), obs.index.max()
        sims = {model + '_' + var: load_data(DATA_DIR.joinpath(storm, 
                                                   plot_type, 
                                                   fc_type, 
                                                   model, 
                                                   station + '.parquet'))
        ...
        return pn.pane.Str(f'{title}: No {plot_type} statistics data')
...
def plot_extremes_table(event):
    ...
    try:
        ...
        title = f"{storm}: {fc_type} at {station}"
        return pn.pane.Str(f'{title}: No {plot_type} extemes data')
